...
> 本文件是全局宪法在本项目的适配层。仓库内 [`AGENTS.global.md`](AGENTS.global.md) 是本项目采用的全局宪法；本文只写项目特化条款，冲突时以更严格者为准。若本地用户级宪法更新，应先审阅差异，再同步更新快照。

## 全局 Skill 根目录

- 用户级 skill 根目录：`C:\Users\Vens_\.agents\skills\`
- 使用 skill 前，必须先读取对应目录下的 `SKILL.md`，并遵循全局宪法与 skill 自身说明。
- skill 仅在任务匹配其描述或用户明确点名时启用；不把 skill 内容复制进项目，除非用户明确要求迁移。

## 基本信息

- 正式名称：`jrpgmaker`（CMake project 名与 C++ 命名空间均为 `jrpgmaker`；早期代号 `jrpgengine` 已废弃，仅存于历史记录）
- 一句话定位：数据驱动、可通过源码级插件扩展战斗规则与渲染风格的 JRPG 3D 运行框架
...
6. **单问题节奏**：每轮对话只解决一个问题。多问题需求必须拆分轮次串行推进；禁止在一轮内混合多个问题的调研、实现与验收。前置预检可以并行收集信息，但结论落地与实现动作每轮只允许属于一个问题。
