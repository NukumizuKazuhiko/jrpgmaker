...
TEST_CASE("editor resources reject unregistered locale and duplicate layout ids", "[ui][editor]") {
    ...
    const auto layout = jrpgmaker::ui::ParseEditorLayout(nlohmann::json{
        {"schema", 1},
        {"id", "editor.workspace"},
        {"root",
         {{"type", "Panel"}, {"id", "root"}, {"children", {{{"type", "Text"}, {"id", "root"}}}}}}});
...
TEST_CASE("editor theme resolves semantic tokens and recipe states", "[ui][editor]") {
    nlohmann::json document = {
        ...
        {"semantic_tokens",
         {{"surface.canvas", "canvas"}, {"surface.panel", "panel"}, {"text.accent", "accent"}}}};
    document["recipes"]["panel"] = {{"normal", "surface.panel"},
                                    {"hover", "surface.panel"},
                                    {"pressed", "surface.panel"},
                                    {"focused", "surface.panel"},
...
TEST_CASE("editor action map rejects duplicate keys and parses resource actions", "[ui][editor]") {
    const auto actions = jrpgmaker::ui::ParseEditorActionMap(
        nlohmann::json{{"schema", 1},
                       {"id", "editor.actions"},
                       {"actions", {{"save", {"Ctrl+S"}}, {"open", {"Ctrl+O"}}}}});
    REQUIRE(actions);
    REQUIRE(actions.value.actions.at("save").front() == "Ctrl+S");
    const auto duplicate = jrpgmaker::ui::ParseEditorActionMap(
        nlohmann::json{{"schema", 1},
                       {"id", "editor.actions"},
...
TEST_CASE("committed editor resources form a valid startup set", "[ui][editor]") {
    ...
    REQUIRE(manifest);
    const auto locale =
        jrpgmaker::ui::ParseEditorLocale(ReadJson(root / "locales/zh-CN.json"), manifest.value);
    const auto theme =
        jrpgmaker::ui::ParseEditorTheme(ReadJson(root / "themes/editor_default.json"));
    const auto high_contrast =
        jrpgmaker::ui::ParseEditorTheme(ReadJson(root / "themes/editor_high_contrast.json"));
    const auto layout =
        jrpgmaker::ui::ParseEditorLayout(ReadJson(root / "layouts/editor_workspace.json"));
    const auto action_map =
        jrpgmaker::ui::ParseEditorActionMap(ReadJson(root / "actions/editor.json"));
    REQUIRE(locale);
    REQUIRE(theme);
...
TEST_CASE("editor startup loader aggregates a complete resource bundle", "[ui][editor]") {
    ...
    REQUIRE(result.bundle->action_map.actions.size() == 13);
