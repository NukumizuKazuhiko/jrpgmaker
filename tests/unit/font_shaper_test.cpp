...
TEST_CASE("text draw resolves localized CJK into glyph quads", "[ui][font][text-draw]") {
    ...
    REQUIRE(source.Add(
        jrpgmaker::ui::DrawText{{10.0f, 20.0f, 100.0f, 30.0f}, "editor.title", {}, std::nullopt}));
    GlyphAtlas atlas(128, 64, 16);
    const auto result = jrpgmaker::ui::BuildTextDrawList(source, locale, font, atlas, 24);
    REQUIRE(result.ok());
    REQUIRE(result.draw_list.size() == 2);
    const auto* glyph =
        std::get_if<jrpgmaker::ui::DrawGlyph>(&result.draw_list.primitives().front());
...
TEST_CASE("text draw projects caret and selection using glyph advances", "[ui][font][text-draw]") {
    ...
    jrpgmaker::ui::DrawList source;
    jrpgmaker::ui::DrawText text{{10.0f, 20.0f, 100.0f, 30.0f}, "editor.value", {}, std::nullopt};
