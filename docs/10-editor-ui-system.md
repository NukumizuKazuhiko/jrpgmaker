...
| `TextField` | UTF-8 文本、selection、caret、IME composition、commit/cancel | P13-1 |
...
- `engine/ui` 输出后端无关 `DrawList`：矩形、裁剪、纹理/图标、glyph run 和 z-order；`tools/editor` 不直接录制 D3D12/Vulkan 命令。
...
2. 提交默认/高对比 theme、`zh-CN`/`en` locale 和 workspace layout fixture，并建立资源 lint。当前资源 fixture 已提交并由 editor 资源合同测试实读验证；独立 lint 命令仍待补齐。
