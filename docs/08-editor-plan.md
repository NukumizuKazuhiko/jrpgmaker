...
- 已实现命令行初始项目路径、工作区加载、字段值/诊断码（含路径）/预览指标的 locale 参数 projection，以及带 bounded 日志的错误面板和资源化状态栏；项目路径选择仍待实现。
...
- P13-0 未闭合不得开始窗口实现；P13-1 至 P13-3 完成后才允许声明“编辑器第一闭环完成”；P13-4/5 未完成时不得声称完整编辑器交付。当前 P13-1 仍有项目路径选择、显式 z-order、caret/selection 绘制和字体预热策略缺口。
