...
- `engine/ui` 已提供有界后端无关 `DrawList`（矩形 recipe/state 与 i18n key），`render::BuildUiDrawPacket` 已将矩形编译为主题驱动的 NDC 顶点/索引上传包，并由 `UploadUiDrawPacket`/`RecordUiDrawPacket` 接入 RHI buffer/indexed draw；窗口 pass 接线、裁剪、glyph run 和 z-order 仍待实现。
