...
当前进度：workspace 的结构化诊断与预览指标已经进入 DrawList，并能通过 locale 参数显示；manifest 驱动的文档标签、诊断 code/path 过滤与文档归属、稳定 diff projection 已接入。workspace/editor 现可在无未保存变更时选择已登记且有 adapter 的数据文档，并通过对应 schema-aware 字段编辑和保存；字段定位、跨文档事务提交仍待完成。
