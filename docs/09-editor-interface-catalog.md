...
以下以当前源码行为为事实；未闭合的目标必须显式标注，不能把目标接口写成已实现合同。
...
角色编辑边界：`engine/core::CharacterController` 当前是运行时移动/碰撞组件，不是项目文档接口；目前没有对应的项目 schema、parser、validator、`DocumentAdapter` 或字段描述。因此本轮不注册“角色调整”菜单项，也不在 GUI 中猜测角色属性。待这些权威合同出现后，才能按同一 document projection seam 接入。
...
- `project::ProjectWorkspace::SelectDocument` 可切换到 manifest 引用且存在 adapter 的文档，并为每个文档保留 working copy；切换后 `CurrentDocument` 与 `CurrentDocumentId` 始终成对更新，Apply/Commit 使用对应文档路径，未保存内容不会因切换丢失。
...
- `FieldDescriptor::choices` 是 adapter 提供的有界、去重候选值；`EditorSession::CycleSelectedChoice` 只接受 `value_type=select`，按确定性顺序循环并重新进入 `ProjectWorkspace::Apply`，候选值为空或当前值不在集合时拒绝。
...
- 真正运行项目由独立 process Adapter 发起，参数数组传递，不拼 shell 字符串；捕获 exit code、启动错误和每路最多 64 KiB 的有界日志，GUI 展示时再限制单行长度。
