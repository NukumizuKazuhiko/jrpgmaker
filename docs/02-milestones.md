...
- **当前状态（2026-08-29）**：P13-0 已开始编码。`engine/ui` 已落地独立 editor manifest/locale/layout/theme/action-map 值对象与 bounded loader，`tools/project` 已提供结构化 workspace `Open`/`Diagnose`/`Apply`/`PrepareSave`/`Commit`/`Migrate` seam，并由 `projecttool open/validate/diagnose/preview/diff/write/migrate` 复用；文档 adapter registry 已覆盖 manifest、事件、导航、碰撞、相机和交互六类文档，`Diagnose` 统一经 adapter 校验后执行交互目标交叉校验；编辑器安装资源已提交默认/高对比主题、`zh-CN`/`en` locale、action map 和 workspace layout fixture，并由 editor 资源合同测试实读验证；`LoadEditorResources` 已聚合启动阶段诊断，`jrpgmaker_editorlint <editor-resource-root>` 已纳入 CTest；`tools/editor` 已建立 SDL host，可选打开 workspace、生成 shell projection、从 action map 构造输入映射并以 locale 标题创建窗口。覆盖 schema、id、locale namespace、重复布局 id、CJK UTF-8 文本、颜色/尺寸/语义 token、recipe 状态、locale 占位符一致性、重复 action key、布局节点上界、revision 冲突、原子备份写回、schema 1 no-op migration、manifest 驱动地图路径及 adapter 重复/非法字段校验，Windows workspace/editor 定向测试、editorlint 和 host smoke 回归通过。实际控件渲染、表单/预览仍未完成，P13-1 尚未闭环。
