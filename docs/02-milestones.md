...
- **当前状态（2026-08-29）**：P13-0 至 P13-2 的公共 seam、编辑器资源、项目 workspace、布局壳、表单/预览 projection 与 RHI UI 绘制路径已落地；`EditorSession` 已把字段选择、类型化 Apply、dirty/revision、Save、UTF-8 文本编辑和表单鼠标命中收拢到 editor adapter，`Form` 布局节点与 focused 控件行已接入 SDL host，`PreviewProcess` 已提供独立 runtime 启动 seam。`engine/ui` 提供 manifest/locale/layout/theme/action-map 值对象、资源驱动字体候选、bounded `DrawList`、占位符参数、FreeType 灰度 glyph atlas 和文本 projection；`render` 已提供独立 sampled-text batch，editor host 已在 Windows D3D12 与 Linux Vulkan 构建中接入 text pipeline。当前验证：Windows 全量 CTest 295/295、Linux/Vulkan 全量 CTest 296/296、Windows editor smoke、editorlint、私有头审计与 `git diff --check` 均通过。P13-2 的文档标签页/诊断筛选/字段定位/diff GUI 与 P13-3 的多文档 schema-aware 编辑已闭合；P13-4 的地图/材质/插件扩展、P13-5 的 GUI 预览结果显示与发布回归仍未闭环；项目路径选择、状态栏/完整错误面板、裁剪、显式 z-order、多字体逐字 fallback、caret/selection 绘制仍是后续阻断项。
