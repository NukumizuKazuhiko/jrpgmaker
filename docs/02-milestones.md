...
- **当前状态（2026-08-29）**：P13-0 已开始编码。`engine/ui` 已落地独立 editor manifest/locale/layout 值对象与 bounded loader，`tools/project` 已提供结构化 workspace `Open`/`Diagnose`/`Apply`/`PrepareSave`/`Commit`/`Migrate` seam，并由 `projecttool open/validate/diagnose/preview/diff/write/migrate` 复用；文档 adapter registry 已覆盖 manifest、事件、导航、碰撞、相机和交互六类文档，`Diagnose` 统一经 adapter 校验后执行交互目标交叉校验；覆盖 schema、id、locale namespace、重复布局 id、CJK UTF-8 文本、布局节点上界、revision 冲突、原子备份写回、schema 1 no-op migration、manifest 驱动地图路径及 adapter 重复/非法字段校验，Windows workspace 定向测试与 projecttool 真实命令回归通过。默认/高对比主题资源和实际 editor 窗口仍未完成，暂不进入 P13-1。
