...
- **状态**：进行中。已落地：ADR-003 shader 选型、RHI 合同 v0 头文件（commit `507105c`）、D3D12 后端骨架（设备/命令队列/围栏/空提交链路，WARP 回退路径，ctest 8/8 双配置通过）。待做：Vulkan 后端骨架、清屏基线 golden 实验、pipeline+三角形、swapchain+SDL3 主循环、CI golden 流水线。
