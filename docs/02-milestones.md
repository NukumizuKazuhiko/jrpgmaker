...
**当前进度（2026-08-28，P10-1 已完成）**：`projecttool create/open/validate` 已扩展为 `diff/write/migrate`。manifest patch 仅允许已登记字段，合并结果必须重新通过 `ParseProjectManifest` 和引用存在性校验；`diff` 输出稳定字段路径与前后值，`write` 先生成临时文件，再保留最多 9 份轮换备份后提交，失败恢复原 manifest；schema 1 的 `migrate` 明确报告无需迁移。Windows 已验证创建、打开、校验、完整 lint、未知字段/路径拒绝、连续写回备份轮换和全量 CTest **225/225**；P10-2 再进入日期/对话/地图等领域数据的 schema-aware 编辑。
