...
- **当前状态（2026-08-29）**：P13-0 至 P13-2 的公共 seam、编辑器资源、项目 workspace、布局壳、表单/预览 projection 与 RHI UI 绘制路径已落地；`EditorSession` 已把字段选择、类型化 Apply、dirty/revision、Save、UTF-8 文本编辑、项目切换和表单鼠标命中收拢到 editor adapter，`Form` 布局节点与 focused 控件行已接入 SDL host，`PreviewProcess` 已提供独立 runtime 启动 seam。`engine/ui` 提供 manifest/locale/layout/theme/action-map 值对象、资源驱动字体候选、bounded `DrawList`、占位符参数、FreeType 灰度 glyph atlas、有序多字体逐字 fallback、真实 glyph advance 驱动的 caret/selection 装饰和文本 projection；`render` 已提供独立 sampled-text batch，editor host 已在 Windows D3D12 与 Linux Vulkan 构建中接入 text pipeline。当前验证：Windows 本轮 P13 定向测试通过；WSL/Linux 全量 CTest 307/307 通过（descriptor/adapter 提交前版本），当前资源/validator 接线需在最终门禁中再跑一轮。P13-2 的文档标签页/诊断筛选/字段定位/diff GUI 与 P13-3 的多文档 schema-aware 编辑已闭合；P13-4 已闭合矩形视口裁剪、plugin editor sidecar 基础清单校验、descriptor 合同解析、到 project adapter 的类型化转换、sidecar 资源存在性校验，以及可注入 registry 的 workspace validator 接线；实际编辑器进程的插件发现/sidecar 装载仍未完成。P13-5 的 GUI 预览结果显示与发布回归仍未闭环；显式 z-order、字体预热策略仍是后续阻断项。
