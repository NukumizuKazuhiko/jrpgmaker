...
- **当前状态（2026-08-29）**：P13-0 已开始编码。`engine/ui` 已落地独立 editor manifest/locale/layout 值对象与 bounded loader，`tools/project` 已提供结构化 workspace `Open`/`Diagnose`/`Apply`/`PrepareSave`/`Commit` seam，并由 `projecttool open/validate/diagnose/preview/diff/write` 复用；覆盖 schema、id、locale namespace、重复布局 id、CJK UTF-8 文本、布局节点上界、revision 冲突和原子备份写回，Windows workspace 定向测试与 projecttool 真实命令回归通过。完整文档 adapter registry、迁移器、默认/高对比主题资源和实际 editor 窗口仍未完成，暂不进入 P13-1。
