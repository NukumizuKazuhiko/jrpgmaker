...
- **当前状态（2026-08-29）**：P13-0 至 P13-2 的公共 seam、编辑器资源、项目 workspace、布局壳、表单/预览 projection 与 RHI UI 绘制路径已落地；`EditorSession` 已把字段选择、类型化 Apply、dirty/revision 和 Save 编排收拢到 editor adapter。`engine/ui` 提供 manifest/locale/layout/theme/action-map 值对象与 bounded loader、backend-agnostic `DrawList`；`tools/project` 提供结构化 workspace `Open`/`Diagnose`/`Apply`/`PrepareSave`/`Commit`/`Migrate` seam，并由 `projecttool open/validate/diagnose/preview/diff/write/migrate` 复用；`tools/editor` 已建立 SDL host，加载主题/locale/layout/action map，创建 Windows D3D12 或 Linux Vulkan 窗口，生成 shell draw list，经 RHI 上传并绘制一帧，同时生成 `FormProjection` 与 `WorkspacePreview` 结构化状态。Windows 全量 CTest 268/268、Linux/Vulkan 全量 CTest 270/270、Windows editor smoke、editorlint、私有头审计与 `git diff --check` 已通过。P13-3 至 P13-5 仍未闭环：可见控件/焦点绘制、GUI 文本输入与字段定位、只读运行时预览启动及发布回归尚待实现；字形图集、裁剪、z-order 和 CJK 实际字形绘制记录为后续 UI 增强债务。
