...
- **当前状态（2026-08-29）**：P13-0 已开始编码。`engine/ui` 已落地独立 editor manifest/locale/layout 值对象与 bounded loader，`tools/project` 已提供结构化 workspace `Open`/`Diagnose`/`Apply`/`PrepareSave`/`Commit`/`Migrate` seam，并由 `projecttool open/validate/diagnose/preview/diff/write/migrate` 复用；文档 adapter registry 已提供默认 `project.manifest` adapter 和字段描述，覆盖 schema、id、locale namespace、重复布局 id、CJK UTF-8 文本、布局节点上界、revision 冲突、原子备份写回、schema 1 no-op migration 及 adapter 重复/非法字段校验，Windows workspace 定向测试与 projecttool 真实命令回归通过。领域文档 adapter、默认/高对比主题资源和实际 editor 窗口仍未完成，暂不进入 P13-1。
