...
- **当前状态（2026-08-29）**：P13-0 至 P13-2 的公共 seam、编辑器资源、项目 workspace、布局壳、表单/预览 projection 与 RHI UI 绘制路径已落地；`EditorSession` 已把字段选择、类型化 Apply、dirty/revision、Save、UTF-8 文本编辑和表单鼠标命中收拢到 editor adapter，`Form` 布局节点与 focused 控件行已接入 SDL host，`PreviewProcess` 已提供独立 runtime 启动 seam。`engine/ui` 提供 manifest/locale/layout/theme/action-map 值对象与 bounded loader、backend-agnostic `DrawList` 及事件/焦点/UTF-8 文本字段状态；`tools/project` 提供结构化 workspace `Open`/`Diagnose`/`Apply`/`PrepareSave`/`Commit`/`Migrate` seam，并由 `projecttool open/validate/diagnose/preview/diff/write/migrate` 复用；`tools/editor` 已建立 SDL host，加载主题/locale/layout/action map，创建 Windows D3D12 或 Linux Vulkan 窗口，生成 shell/form draw list，经 RHI 上传并绘制一帧，同时生成 `FormProjection` 与 `WorkspacePreview` 结构化状态。当前提交已通过 Windows 编辑器针对性 21/21、Linux 编辑器与文本针对性 7/7，Windows 全量 CTest 278/278、Linux/Vulkan 全量 CTest 278/278、Windows editor smoke、editorlint、私有头审计（175 文件）与 `git diff --check`。P13-3 至 P13-5 仍未闭环：实际字形绘制/可读文本、诊断与预览 projection 的可见面板、GUI 运行预览结果展示、发布回归和完整 IME composition 接线尚待实现；字形图集、裁剪、z-order 和 CJK 实际字形绘制仍是阻断项。
