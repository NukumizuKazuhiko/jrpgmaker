...
**当前进度（2026-08-28，P12 Windows/Linux 验收中）**：已新增基于真实 demo 数据的 acceptance test，覆盖移动/碰撞、A*、交互提示与确认、事件/对话、日程和存档恢复；Windows/MSVC 与 WSL/Linux 均完成两轮本地构建和全量 CTest，当前为 **233/233**。数据 lint、golden-sync、shader 权威 artifact 一致性、私有头审计和发布包确定性均已验证；发布包已修复误收 `CMakeFiles`、对象文件和 CMake 元数据的问题。新增用户指南并登记至文档索引。macOS 按既定决策延后，不阻塞本轮；仍需从提交基线复核一次最终 Git 边界，并将可复现证据整理为稳定版本验收记录。
