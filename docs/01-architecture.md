...
- **骨骼动画核心 owner = `core/animation.hpp`（P4 子任务 1 落地，纯逻辑无 RHI 依赖）**：`Skeleton`（joint 层级 + inverse-bind + 节点静态 TRS 基础姿态，≤kMaxBones=32 构造期校验）、`SamplePose`（静态 TRS 起底 + 动画通道覆盖，LINEAR=slerp / STEP / CUBICSPLINE=Hermite 含 dt 缩放与归一化）、`BlendPose`（T/S lerp + R slerp——双 clip 权重混合即 v0 混合树）、`BoneMatrices`（局部 TRS 沿 parent 链前向单遍合成全局 × IB，输出列主序 mat4 数组直供 uniform buffer）。渲染层只消费 `BoneMatrices` 输出，禁止私造蒙皮语义。
