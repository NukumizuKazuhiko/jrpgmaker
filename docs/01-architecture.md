...
```
SDL3 事件 → editor host → RmlUi UI 控件命令
                         ↓
...
- `tools/editor::RmlUiEditorView` 拥有编辑器 DOM/RCSS、布局、焦点、事件与 RHI 绘制适配；`engine/ui` 继续拥有运行时通用控件行为与渲染无关状态。两者都不拥有项目数据。
...
- **golden 流水线（P1 落地版）**：基准图（`tests/golden/*.ppm`，二进制 PPM P6，RGB 每像素 3 字节）由 **lavapipe（Linux CI 权威环境）生成并提交入库，生成物只读**——变更必须经 `tools/goldenimage` CLI 重新生成后提交，禁止手改。`jrpgmaker_goldenimage` CLI 支持 `generate <out.ppm>`（渲染三角形→写基准图）与 `compare <ref.ppm> [tolerance]`（渲染→全帧逐像素比对，输出 diff 统计与退出码）；纯算法（PPM 读写、RGBA8 全帧比对）在 `jrpgmaker_golden` 静态库（`tools/goldenimage/golden_image.{hpp,cpp}`，无 RHI 依赖）。CI 的 golden-sync job（Linux lavapipe）重新生成基准图→`git diff --exit-code` 作为门禁（与 shader-sync 同模式），并把基准图上传为 artifact（截图产物）。runtime overlay CJK 的主图与 `runtime_overlay_cjk_256x160_edge_class.ppm` 因果 mask 由 `tools/ci/generate_runtime_overlay_golden.ps1` 成对调用真实 Catch2 GPU readback 场景生成，二者同样纳入 golden-sync 漂移检查与 artifact；生成器要求两个输出路径不同并恢复两个环境变量。测试先对当前 GPU mask 与 lavapipe 权威 edge-class reference 做全帧零容差比较，再对主图使用每通道 tolerance=1，且只有 reference mask 的白色 partial-coverage glyph 像素允许该 1 级差异；mask 黑色像素及非文字区域必须 bit-exact，不使用某台机器的差异像素数量作为合同。该唯一受限 CJK alpha composite 合同对应 D3D12/Vulkan 对 RGBA8 UNORM 固定功能 blend 的已复现实作量化差异，不适用于其他 golden。**跨驱动容差**：v0 三角形在 WARP 与 lavapipe 下实测 max channel delta=0（纯色、无抗锯齿、64×64 光栅化规则一致），triangle_test 用 tolerance=2 保留余量；未来引入插值/抗锯齿场景时须重新标定（见 docs/02 §P1 预检"头号技术风险"）。三角形顶点非对称（上方顶点、下方底边），全帧比对天然锁定 Vulkan 负高度 viewport 的 NDC-Y 约定（DEBT-016 闭环）。
...
- 场景导入不缓存（v0 每次调用独立 parse+load）；**异步加载（P2 子任务 7 / P8-3 扩展）**：assetimport `AsyncLoader` 后台线程只做 glTF 解析或独立纹理 CPU 解码，`Poll()` 在调用方线程派发完成回调并按需注册进 `AssetRegistry`/排队 RGBA8 upload packet——后台线程禁止触碰 RHI；注册表与 `TextureResourceService` 的 GPU 操作均保持在各自 owner 线程边界内。请求容量包含 queue、in-flight 和 finished 结果，调用方必须持续 Poll。
...
- **对话投影**：`DialogRequested` 即结构化投影（含 event_id/speaker/text_key/options，choice 有、dialog 空），供 UI 消费——2026-08-25 审计移除冗余的 `DialogState` 死结构体（声明从未发布），合同统一为单一投影事件。`ui::DialogPresentation` 消费该投影和项目本地化表，解析显示文本并维护当前高亮选项；app 只把 `dialog.previous` / `dialog.next` / `extension.confirm` 输入命令映射为选择和 `AdvanceDialog(index)`，不解释分支。打字机进度、立绘槽位与 GPU 字形绘制仍属于 presentation 后续实现。
...
| 编辑器 UI | RmlUi Core（CMake FetchContent，固定审计 commit；仅 `tools/editor`） | 提供 retained-mode DOM、RCSS、布局、焦点、控件和事件路由；通过项目自有 `RenderInterface` 接入现有 RHI，不进入 engine/domain/runtime 依赖 |
