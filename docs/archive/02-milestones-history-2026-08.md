# 里程碑与验收门禁（2026-08 历史证据）

> 状态：历史证据，已由 [`../02-milestones.md`](../02-milestones.md) 取代。本文保留 2026-08 的实施与审计记录，不再作为当前里程碑状态真源。

## 全局门禁（P0 起对每个阶段生效）

1. 当前支持平台 Windows/Linux 的 build + test 绿灯。工作流中现存的 macOS job 仅作历史兼容性检查，不进入当前产品完成定义；其工作流失败语义不在本文档整理任务中调整。
...
- **执行顺序**：P6-1 render seam 与资源预算 → P6-2 材质 schema 委派与 glTF material 数据保留 → P6-3 基础无光照插件 → P6-4 第二风格插件 → P6-5 cutscene/UI/audio adapter → P6-6 双插件 golden 替换实验与当前支持平台门禁。
...
- **停止条件**：完成 Windows/Linux 编译与测试验证；macOS 不属于当前停止条件，后续适配时单独验收。
...
- **验收**：一次 CLI 命令校验项目、插件私有数据、跨文件引用和资源依赖；坏数据按文件/字段路径聚合并以非零退出；只修改数据即可替换日期、对话、输入、地图、材质和插件；相同输入生成稳定清单与派生物摘要；完成 Windows/Linux 当前平台门禁。
...
- **范围内**：公开插件 SDK 文档与模板；插件 manifest/contract 兼容策略；插件 validator、资源和 presentation 扩展点；构建期注册的可重复装配；错误隔离、预算/超时/队列上界；存档迁移矩阵；性能基线、GPU/CPU/内存预算和长时间运行测试；Windows/Linux 发布包与排障文档。
...
- **验收**：第三方最小插件仅依赖公开合同即可构建、注册、校验、运行和卸载；替换战斗插件与渲染风格插件不改 `engine/domain`、RHI 后端或 app 业务分支；长时间运行无资源泄漏、无无界队列和无未处理结构化错误；Windows/Linux 的构建、测试、数据 lint、golden、shader-sync、格式和私有头门禁全绿。
...
> **平台边界修订（2026-08-29）**：上方 2026-08-28 进度中的 macOS 延后项不再属于 P11 待办或完成门禁。P11 只按 Windows/Linux 当前支持平台验收；历史 macOS CI 结果保留为兼容性证据，不构成当前支持承诺。
...
- **最终门禁**：产品成功标准 7 项全部有真实证据；Windows/Linux 当前支持平台门禁与可运行环境验收通过；全量测试、数据 lint、golden/shader-sync、格式、私有头审计和依赖许可清单通过；发布包可在干净环境安装运行；文档索引、架构、里程碑、债务登记和用户指南同步到同一版本。macOS 后续适配不阻断当前完成定义。
...
**当前进度（2026-08-28，P12 Windows/Linux 主线完成，macOS 延后）**：已新增基于真实 demo 数据的 acceptance test，覆盖项目 manifest 解析、渲染风格/战斗插件注册与实例化、插件数据校验，以及移动/碰撞、A*、交互提示与确认、事件/对话、日程和存档恢复；Windows/MSVC 与 WSL/Linux 均完成本地及独立干净 checkout 构建，提交基线全量 CTest 均为 **233/233**。数据 lint、golden-sync、shader 权威 artifact 一致性、私有头审计和发布包确定性均已验证；发布包已修复误收 `CMakeFiles`、对象文件和 CMake 元数据的问题，并在 Windows 干净 checkout 上完成两次装配，37 个运行时文件清单 SHA-256 一致且中间文件为 0。新增用户指南并登记至文档索引。按当前平台决策，Windows + Linux 的 P12 稳定版本验收已完成；macOS 保留为后续适配，不阻塞本轮。

> **P12 状态校正（2026-08-29）**：上方记录证明 Windows/Linux 的构建、发布与自动化验收主线已完成；macOS 已移出当前完成定义。P12 整体仍须以 [00-product.md](../00-product.md) 的成功标准为准，当前尚缺真实 30 分钟可玩证据与中日文 golden image，因此不得仅凭平台门禁宣称首个稳定版本全部完成。

## P13 开发者项目编辑器 GUI

- **目的**：在 P10 CLI 装配闭环之上提供可选的本地 GUI 编辑器，降低项目作者直接编辑 JSON/资源清单的成本，同时保持 parser、validator、迁移器、CLI 和运行时合同为真源。
- **范围与阶段**：详见 [编辑器 GUI 计划](../08-editor-plan.md)，先完成 P13-0 公共 seam/主题/i18n 合同，再按 P13-1 编辑器壳、P13-2 文档模型与诊断、P13-3 schema-aware 表单、P13-4 地图/材质/插件扩展、P13-5 只读预览与发布回归推进。
...
- **当前状态（2026-08-29）**：P13-0 至 P13-3 的公共 seam、编辑器资源、项目 workspace、布局壳、表单/预览 projection 与 RHI UI 绘制路径已落地；`EditorSession` 已把字段选择、类型化 Apply、dirty/revision、Save、UTF-8 文本编辑、项目切换和表单鼠标命中收拢到 editor adapter，`Form` 布局节点与 focused 控件行已接入 SDL host，`PreviewProcess` 已提供独立 runtime 启动 seam。`engine/ui` 提供 manifest/locale/layout/theme/action-map 值对象、资源驱动字体候选、bounded `DrawList`、占位符参数、FreeType 灰度 glyph atlas、有序多字体逐字 fallback、真实 glyph advance 驱动的 caret/selection 装饰和文本 projection；`render` 已提供独立 sampled-text batch，editor host 已在 Windows D3D12 与 Linux Vulkan 构建中接入 text pipeline。P13-4 已闭合地图/材质数据 adapter、plugin editor sidecar/descriptor 合同、资源 containment 与大小上界、workspace validator 接线、插件文档有界枚举、类型化编辑、标签页投影和原子保存。P13-5 已闭合只读预览 projection、独立 runtime 启动状态、跨文档发布回归及 Windows/Linux 验收。当前验证：Windows 全量 CTest 310/310、Linux/WSL 全量 CTest 311/311、P13 定向测试 43 assertions/9 cases、私有头审计 179 文件和 `git diff --check` 均通过。显式 z-order 与字体预热策略作为后续体验增强，不阻断 P13 闭环。

> **本轮 P13 闭环验证（2026-08-29）**：Windows 增量构建与 `ctest --preset win-debug --output-on-failure` 为 **310/310**；WSL/Linux 增量构建与 `ctest --preset linux-debug --output-on-failure` 为 **311/311**；P13 定向测试为 **43 assertions/9 cases**；`pwsh ./tools/ci/check_private_headers.ps1` 扫描 179 个文件通过，`git diff --check` 通过。WSL 无 WSLg，未将 SDL 实窗口验证伪装成本地通过；macOS 仍按平台边界延期。
...
| 2 | 后续 macOS/MoltenVK 适配污染当前合同 | 中 | 仅经现有 Vulkan RHI 接入；独立验收且不得回退 Windows/Linux 门禁或引入平行业务语义 |
